    ...
    <div>
      ...
      <div className="tc-card">
        ...
        <ul>
          ...
          <li>
            地图<strong>不对省份做数据着色</strong>（早期版本的「线索密度」配色已删除）：省份只作为可缩放的地理参照，
            信息通过医院节点与悬停提示给出，避免用颜色暗示地区差异。
          </li>
          <li>
            医院节点的坐标为<strong>省级近似位置</strong>（`coordinates_source: province-centroid`），
            同省机构按环形展开，<strong>不是门址</strong>；拿到地图服务 key 后会升级为真实地理编码坐标。
          </li>
          <li>节点数量只表示「我们收集到多少公开线索」，<strong>不是确诊人数统计</strong>，也不能用于地区间比较。</li>
        </ul>
      </div>
