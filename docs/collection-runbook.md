...
### 0.9 三条硬教训（2026-09-11，必读）

1. **签名 URL（`xsec_token`）会过期（数小时）**。过期后详情/评论页不会渲染，抓取会「成功但 0 条」。
   对策：`sweep.ts` 每次重新见到笔记都会刷新 URL；要抓评论前**先跑一次 sweep** 刷新 token，再立刻抓。
2. **空结果不得覆盖已有数据**。`fetch-comments.ts` 已加保护：抓到 0 条且旧文件非空时保留旧数据并告警。
3. **摘录必须带上下文**，规则见 [evidence-standards.md](./evidence-standards.md)：
   评论要交代所在帖子与父评论；原文未点名的机构不得建档；地区不得臆测。
   入库后跑 `npm run collect:backfill` 回填上下文，跑 `npm run data:validate` 验证。
